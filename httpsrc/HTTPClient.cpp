...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
    ...
    HTTPClientSetVerb((unsigned long)pHTTPSession,(HTTP_VERB)HTTP_CLIENT_DEFAULT_VERB);    // Default HTTP verb
	...
	return (unsigned long)pHTTPSession;
}
    ...
    UINT32 *nBytesRecived)   // [OUT]    Count of the bytes that ware received in this operation
{
    ...
    *pNullPtr = 0;
    ...
    if(nRetCode == HTTP_CLIENT_SUCCESS)
    {
        ...
        if(pHTTPSession->pDebug)
        {
            pHTTPSession->pDebug("HTTPClientReadData",NULL,0,"Reading %d bytes",nBytes, 0);
        }
#endif
...
///////////////////////////////////////////////////////////////////////////////
 ...
 UINT32 HTTPIntrnResizeBuffer (P_HTTP_SESSION pHTTPSession,UINT32 nNewBufferSize)    // [IN] The new (and larger) buffer size
{
...
///////////////////////////////////////////////////////////////////////////////
 ...
 UINT32 HTTPIntrnHeadersRemove (P_HTTP_SESSION pHTTPSession, CHAR *pHeaderName)   // [IN] The header's name
 ...
 UINT32 HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
{
    ...
    do
    {
       ...
       // nRetCode = fcntl(pHTTPSession->HttpConnection.HttpSocket,  F_SETFL, (u_long*)&nNonBlocking);
	   nRetCode = fcntl(pHTTPSession->HttpConnection.HttpSocket,  F_GETLK , (u_long*)&nNonBlocking);
        if(nRetCode != 0)
        {
        ...
        while(1)
        {
            ...
            if(nSocketEvents == 0)
            {
                ...
                {       
                    ...
                    if(nRetCode > 0)
                    {
                        ...
                        if((nRetCode = HTTPWrapperSSLRecv(pConnection->HttpSocket,pData,*(nLength),0)) == -1)
                        {
                            // Socket error
            ...
            if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDRead)) // Are there any read events on the socket ?
            {
                ...
                {     
                    ...
                    {
                        if((nRetCode = recv(pConnection->HttpSocket,pData,*(nLength),0)) == -1)
                        {
                            // Socket error
                            nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
                            break;
                ...
                {
                    // Only peek te socket
                    if((nRetCode = recv(pConnection->HttpSocket,pData,*(nLength),MSG_PEEK)) == -1)
                    {
                        // Socket error
                        nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
                        break;
    ...
    do
    {
        ...
        if(pHTTPSession->HttpHeaders.HttpVerb != HttpVerb)
        {
           // if((nRetCode = HTTPClientSetVerb((HTTP_SESSION_HANDLE)pHTTPSession,HttpVerb)) != HTTP_CLIENT_SUCCESS)
		    if((nRetCode = HTTPClientSetVerb((unsigned long)pHTTPSession,HttpVerb)) != HTTP_CLIENT_SUCCESS)
            {
                break;
            }
        }
        ...
        if(pHTTPSession->HttpHeaders.HttpVerb != HttpCachedVerb)
        {
			...
			if((nRetCode = HTTPClientSetVerb((unsigned long)pHTTPSession,HttpCachedVerb)) != HTTP_CLIENT_SUCCESS)
            {
                break;
            }
        }
 ...
 UINT32 HTTPIntrnSessionReset (P_HTTP_SESSION pHTTPSession, BOOL EntireSession)
{
    ...
    if(EntireSession == TRUE) // Partial reset, clear only the incoming headers
    {
        ...
        // Set default values in the session structure
        HTTPClientSetVerb((unsigned long)pHTTPSession,(HTTP_VERB)HTTP_CLIENT_DEFAULT_VERB);    // Default HTTP verb zjw
        pHTTPSession->HttpUrl.nPort             = HTTP_CLIENT_DEFAULT_PORT;             // Default TCP port
        // Set the outgoing headers pointers
